...
use super::{
    ...
    state::{
        ...
        ThreadActionItem, presence_color, presence_marker,
    },
};
use crate::discord::{
...
const MENTION_ORANGE: Color = Color::LightRed;
...
fn active_text_style(active: bool, style: Style) -> Style {
    if active {
        style.fg(Color::Gray).add_modifier(Modifier::BOLD)
    } else {
        style
    }
}
...
fn render_vertical_scrollbar(
    ...
    let scrollbar = Scrollbar::new(ScrollbarOrientation::VerticalRight)
        ...
        .track_style(Style::default().fg(Color::LightBlue));
...
fn channel_unread_decoration(
    ...
    match unread {
        ChannelUnreadState::Mentioned(count) => {
            let style = base.fg(MENTION_ORANGE).add_modifier(Modifier::ITALIC).add_modifier(Modifier::BOLD);
            (Some(Span::styled(format!("({count}) "), style)), style)
        }
        ChannelUnreadState::Notified(count) => {
            let style = base.fg(UNREAD_BRIGHT).add_modifier(Modifier::ITALIC).add_modifier(Modifier::BOLD);
            (Some(Span::styled(format!("({count}) "), style)), style)
        }
        ChannelUnreadState::Unread => (None, base.fg(UNREAD_BRIGHT).add_modifier(Modifier::ITALIC).add_modifier(Modifier::BOLD)),
...
fn panel_block_owned(title: String, focused: bool) -> Block<'static> {
    let border = if focused { Color::LightRed } else { Color::LightBlue };
